(function (window) {
    ...
    Default constructor
    */
    var _CSV = function (JSONData, cuName) {
        ...
        var csvData = typeof JSONData != 'object' ? JSON.parse(JSONData) : JSONData,
            ...
            BREAK = '\r\n',
            DELIMITER = ';',
			FILENAME = "export"+cuName+".csv";

        // Get and Write the headers
        csvHeaders = Object.keys(csvData[0]);
        csvOutput += csvHeaders.join(DELIMITER) + DELIMITER + BREAK;
        var text;
        
        for (var i = 0; i < csvData.length; i++) {
            var rowElements = [];
            for(var k = 0; k < csvHeaders.length; k++) {
              text = csvData[i][csvHeaders[k]] != null ? csvData[i][csvHeaders[k]] : '';
              text = text.replace(/"/g, '\'');
              rowElements.push('"' + text + '"');
            } // Write the row array based on the headers
            csvRows.push(rowElements.join(DELIMITER));
        }

        csvOutput += csvRows.join(DELIMITER + BREAK);
